...
fig,ax=plt.subplots(figsize=(10,4.2));x=np.arange(4);keys=['accuracy','macro_f1','oos_recall','oos_precision']
...
ax.set(xticks=x,xticklabels=['Accuracy','Macro-F1','Out-of-scope recall','Out-of-scope precision'],ylabel='Percent',ylim=(0,109));tidy(ax)
ax.legend(frameon=False,loc='upper center',bbox_to_anchor=(.5,-.11),ncol=2,fontsize=10);save(fig,'06-route-accuracy')
...
ax.set(xlabel='Seconds per request, client-observed through OpenRouter (log scale)',ylabel='Requests');tidy(ax)
...
for ax,k,title,col in zip(axes,['jev','luna'],['Jev 1.13 · probability of its choice','GPT-6 Luna · stated confidence'],[JEV,LUNA]):
 ...
 ax.set(xlabel='Stated probability the route is right',title=title,xlim=(-.04,1.04),ylim=(-.05,1.08));tidy(ax)
...
ax.set(xlabel='Share of requests routed automatically (the rest go to a person)',ylabel='Accuracy on routed share (%)',xlim=(0,101),ylim=(84,101));tidy(ax)
...
ax.set(xlabel='Cost per 1,000 questions, router plus answering model (USD)',ylabel='Questions answered correctly (%)',ylim=(55,92));tidy(ax)
...
for ax,k,title in zip(axes,['pJev','pLuna'],['Jev 1.13','GPT-6 Luna']):
 ...
 ax.set(title=title,xlabel='Predicted probability that Luna answers correctly');tidy(ax);ax.legend(frameon=False,fontsize=9,loc='upper left')
